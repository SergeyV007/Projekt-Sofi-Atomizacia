...
test.describe('test login', () => {
    test('sergey', async ({page}) => {
        await page.goto('https://saucedemo.com') // Setup - Precondition
...
function validateCredentials(username, password) {
    ...
    if (!username || !password) {
        console.error(
            'Проблема с логином или паролем: поля не должны быть пустыми.',
        )
    ...
    if (username.length < MIN_LENGTH || password.length < MIN_LENGTH) {
        console.error(
            'Проблема с логином или паролем: длина должна быть не менее 6 символов.',
        )
        return false // Прерываем выполнение
    }
    ...
    if (username !== VALID_USERNAME || password !== VALID_PASSWORD) {
        console.error(
            'Проблема с логином или паролем: неверные учетные данные.',
        )
        return false // Прерываем выполнение
    }
